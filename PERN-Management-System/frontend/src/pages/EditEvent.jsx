...
function EditEvent({ events, onUpdate }) {
  // Use React Router hooks to get URL parameters and navigate pages
  const { id } = useParams();
  const navigate = useNavigate();

  // Find the event in the events list that matches the ID in the URL
  // We parse the ID to an integer because route params are always strings
  const eventId = parseInt(id);
  const existingEvent = events.find(event => event.id === eventId);

  // Initialize form state with the existing event data if found, or empty strings if not
  const [formData, setFormData] = useState(
    existingEvent 
      ? { ...existingEvent } 
      : { event_name: "", venue: "", date: "", coordinator: "", description: "" }
  );

  // Local state for validation error alerts
  const [error, setError] = useState("");

  // If the event wasn't found (e.g. invalid ID in URL), show an error layout
  if (!existingEvent) {
    return (
      <div className="max-w-md mx-auto py-12 text-center space-y-6">
        ...
        <div className="space-y-2">
          <h2 className="text-xl font-bold text-slate-800 tracking-tight">Event Not Found</h2>
          <p className="text-slate-500 text-sm">
            The event with ID {id} does not exist or may have been deleted.
          </p>
        </div>
        <Link
          to="/events"
          className="inline-flex items-center gap-1.5 px-4 py-2 bg-indigo-600 hover:bg-indigo-500 text-white font-semibold rounded-xl text-sm transition-colors"
        >
          <ArrowLeft size={16} />
          Back to Events List
        </Link>
  ...
  const handleSubmit = (e) => {
    ...
    // Call the parent handler function passed via props to update the state
    onUpdate(formData);
    ...
    <div className="max-w-2xl mx-auto space-y-6 py-4">
      ...
      <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6 md:p-8 space-y-6">
        ...
        <div className="flex items-center gap-2 pb-4 border-b border-slate-100">
          ...
          <div>
            <h2 className="text-xl font-bold text-slate-800 tracking-tight">Edit Freshers Event</h2>
            <p className="text-slate-500 text-xs mt-0.5">Modify the published details of this event.</p>
          </div>
        </div>
